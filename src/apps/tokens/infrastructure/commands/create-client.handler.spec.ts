...
describe('Create Client Command Handler', () => {
  ...
  beforeAll(async () => {
    app = await Test.createTestingModule({
      imports: [CqrsModule],
      providers: [CreateClientHandler, { provide: 'ClientServiceInterface', useValue: clientServiceMock }]
    }).compile();
  ...
  it('"CreateClientHandler::execute": should create the Client from name and scope', async () => {
    const serviceSpy = jest.spyOn(clientServiceMock, 'create');

    await handler.execute(new CreateClientCommand('client-1', '["app-1", "app-2"]'));
